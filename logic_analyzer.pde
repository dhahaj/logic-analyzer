...
float reducer = 1.0, xShift;
boolean milliseconds = true;

// start point in the processing window
int xEdge = 60, yEdge = 30, xEnd;
float[] xPos = {
  0, 0, 0, 0, 0, 0
};
int yBottom, yDiff, yPos = yEdge, ySave = yEdge;
boolean textCovered, drawTimes = true;

//Serial from mcu
//initial data
int samples, event, initialState;
boolean first = false, dataComplete = false;
boolean [][] state;
...
//buttons and others
int button1X = 8, button2X = 8, button3X = 80, button4X = 200, button5X = 270, buttonY, buttonH = 20;
int smallButtonW = 50, bigButtonW = 100, graphBoxH, textBoxH, image = 1, corner = 10, handleFill = grey;
float handleX, handleY, handleW = 20, handleH = 15;
...
void setup () {
  ...
  if (p==null) {
    ...
    JOptionPane.showMessageDialog(null, "Available COM ports", "COM Ports", JOptionPane.INFORMATION_MESSAGE);
    String port = JOptionPane.showInputDialog("Enter COM Port number");
    port = "COM"+port;
    LA_port = port;
  }
  while (p==null) {
...
void mouseClicked() {
  ...
  // draw times
  if (mouseY>yBottom-15 && mouseY <yBottom-15+buttonH && mouseX>button1X && mouseX <button1X+smallButtonW) {
    drawTimes = !drawTimes;
    getData();
  }
  ...
  //save frame
  if (mouseY>buttonY && mouseY <buttonY+buttonH && mouseX>button5X && mouseX <button5X+smallButtonW) {
    String a = "la_capture-"+image; //+".jpg";  //if you prefer this format, default .tif
    save(a);
    image++;
  }
}
...
void mouseWheel(MouseEvent event) {
  ...
  if (mouseY>buttonY && mouseY <buttonY+buttonH && mouseX>button4X && mouseX <button4X+smallButtonW) {
...
float[] spacing = {
  5, 8
};  //used for the dashline function, pixels
...
void dashline(float x0, float y0, float x1, float y1, float[] spacing) {
  ...
  {
    int i;
    boolean drawLine = false; // alternate between dashes and gaps
